...
public class HTMLObject {
    ...
    public String getHtmlSourceAsHtml() {
        String a = htmlSource.replaceAll(TAG_END, TAG_END + CRLF);
        if (a.isEmpty() && inlineTag) {
            a = str_start;
        }
        return a;
    }
    ...
    public ArrayList<HTMLObject> getObjectsById(String id) {
        ArrayList<HTMLObject> aux = new ArrayList<>();
        for (HTMLObject in : allObjects) {
            if (in.getStr_start().contains("id=\"" + id + "\"")
                    || in.getStr_start().contains("id='" + id + "'")) {
                aux.add(in);
            }
        }
    ...
    public String getAttribute(String attrb) {
        String ret = null, aa = getHtmlSourceAsHtml();
        if (aa.contains(attrb)) {
            try {
                String[] arr = aa.split(attrb);
                char item = arr[1].toCharArray()[1];
                arr = arr[1].split(String.valueOf(item));

                ret = arr[1];
                if (attrb.equals("href")) {
                    ret = ret.replaceAll("&amp;", "&");
                }
            } catch (Exception e) {
            }
    ...
    public ArrayList<HTMLObject> getObjectsByClass(String clas) {
